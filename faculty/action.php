    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          ?>
          <form method="post">
                    <h1 class="h3 mb-4 text-gray-800">Edit the tags of the selected document.</h1><hr>
                    <?php
                    $sql = "select * from files where id='".$_GET['id']."'";
                    $resul = mysqli_query($db, $sql);
                    $row=mysqli_fetch_array($resul);
                    ?>
                    <input type="text" class="form-control" value="<?php echo $row['tags'] ?>" name="tags">
                    <input type="hidden" name="id" value="<?php echo $_GET['id'] ?>"><br>
                    <input type="submit" class="btn btn-primary" name="sub" value="Change">
                    <?php
                    if (isset($_POST['sub'])) {
                      $sql = "UPDATE `files` SET `tags`='".$_POST['tags']."' WHERE id='".$_POST['id']."'";
                      if (mysqli_query($db,$sql)) {
                        echo "<script>alert('we have succesfully updated the tags');</script>";
                      }
                      else
                        echo "<script>alert('we are not able to update the tags');</script>";
                    }
                    ?>
               </form>     
...
if (isset($_GET['delete'])) {
  ...
  if (mysqli_query($db,$sql)) {
    ?>
    <h1 class="h3 mb-4 text-gray-800">You have succesfully deleted the documents.</h1>
    <?php
  }
}
?>
